...
// calc ran.
function installFakeCalc(
  descriptor: any,
  basename = '__test_nogroup_calc'
): () => void {
  ...
  return function cleanup() {
    try {
      ...
      // ignore — test may race with cleanup when the file was never
...
function serialiseDescriptor(obj: any): string {
...
describe('plugin index.js — schema/uiSchema', () => {
  const makePluginApp = (): any => {
    const streams: Record<string, any> = {}
    return {
      selfId: 'self',
      streambundle: {
        getSelfStream: (p: string) => {
          if (!streams[p]) {
...
describe('plugin index.js — updateOldTrafficConfig', () => {
  it('migrates legacy notificationRange/notificationTimeLimit into notificationZones', () => {
    ...
    const app: any = {
      ...
      registerDeltaInputHandler: () => {},
      signalk: { self: 'vessels.self' },
      savePluginOptions: (props: any) => saved.push(props)
    }
...
describe('plugin index.js — nogroup and function-properties calcs', () => {
  ...
  it('honours an explicit calc.ttl override even when props.default_ttl is 0', (done) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const plugin = require('../../src')(busApp)
    plugin.start({
